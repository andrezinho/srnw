<?php
    ...
    $FormatoGrilla = array ();
    /*$Sql = "SELECT atencion.idatencion, sql_fecha(atencion.fecha, dd/mm/yyyy), c.nombres||' '||c.ape_paterno||' '||c.ap_materno, atencion_estado.descripcion, atencion.idusuario, atencion.estado FROM atencion_estado INNER JOIN atencion ON (atencion_estado.idatencion_estado = atencion.estado) inner JOIN cliente c ON c.idcliente = atencion.idcliente";*/
    $FormatoGrilla[0] = "";             											//Sentencia SQL
...
function editkardex(idk){ 
    location.reload('../kardex/index.php'); 
    $("#Modificar").dialog("open");
    $("#Modificar").html("<center><img src='../../imagenes/avance.gif' width=20 /></center>");
    
    $.ajax({
        url:'../kardex/Mantenimiento.php',
        type:'POST',
        async:true,
        data:'Op=1&Id=' + idk,
        success:function(data){ 
    ...
    //location.href='../kardex/index.php';
}   
function Validar(){
...
function Buscar(Op){
    var busca=encodeURIComponent($('#Valor').val());
    $.ajax({
        url:'grilla.php',
        type:'POST',
        async:true,
        data:'Pagina=' + Pagina + '&Formato=<?php echo serialize($FormatoGrilla);?>'+'&Valor=' + busca + '&Anio=' + $('#Anio').val(),
        success:function(data){
