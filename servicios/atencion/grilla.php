<?php
$Sql = "SELECT atencion.idatencion, sql_fecha(atencion.fecha, dd/mm/yyyy), c.nombres||' '||COALESCE(c.ape_paterno,'')||' '||COALESCE(c.ap_materno,''), atencion_estado.descripcion, atencion.idusuario, atencion.estado FROM atencion_estado INNER JOIN atencion ON (atencion_estado.idatencion_estado = atencion.estado) inner JOIN cliente c ON c.idcliente = atencion.idcliente";
...
if(!session_id()){ session_start(); }
    ...
    $Campo = $Campo."AND atencion.idnotaria='".$_SESSION["notaria"]."' ";
    $Sql    = $Conn->VerificarSQL($Sql.$Campo);
    //$n = ") ILIKE '%'".$Valor."'%' )";

    //$pos = strpos($Sql, $n);
    //$Sql = substr_replace($Sql,"",$pos+strlen($n),0);
    //AND atencion.idnotaria='".$_SESSION["notaria"]."' 
    //echo $Sql;
