...
if ( strlen ($_SESSION['id_user'])>0 ) {
    ...
    $Campo = $Campo." AND kardex.idnotaria='".$_SESSION["notaria"]."' AND kardex.estado!=3";
    $Sql    = $Conn->VerificarSQL($Sql.$Campo);
    //$n = ") ILIKE '%$Valor%' )";
    //$pos = strpos($Sql, $n);
    
    //$Sql = substr_replace($Sql," AND kardex.idnotaria='".$_SESSION["notaria"]."' AND kardex.estado!=3 ",$pos+strlen($n),0);    
...
<?php
            ...
            $Sql = "SELECT login FROM usuario WHERE idusuario=".$row[6];
...
<?php			
        for ($ii=1; $ii<=$FormatoGrilla[9]['NB']; $ii++){
                if ($FormatoGrilla[9]['BtnCC'.$ii]=='=='){
                        if(!isset($row[$FormatoGrilla[9]['BtnCI'.$ii]-1]))$row[$FormatoGrilla[9]['BtnCI'.$ii]-1]='';
                ...
                if (($Condicion && $row[6]==$_SESSION['id_user'] || 1==$_SESSION['Admin']) || ($FormatoGrilla[9]['BtnId'.$ii]=='BtnVer')){?>
