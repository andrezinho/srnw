...
if(!session_id()){ session_start(); }
    ...
    if ($Anio!=''){
        $Campo = $Campo." AND carta.anio='$Anio'";
    }
    $Campo = $Campo." AND carta.idnotaria ='".$_SESSION['notaria']."' AND carta.estado!=3";
    $Sql    = $Conn->VerificarSQL($Sql.$Campo);
    
    //$n = ") ILIKE '%$Valor%' )";
    //$pos = strpos($Sql, $n);
    //$Sql = substr_replace($Sql," AND carta.idnotaria ='".$_SESSION['notaria']."' AND carta.estado!=3 ",$pos+strlen($n),0);
    //$Sql .= " ORDER BY carta.idcarta ";
...
$Sql = "SELECT login FROM usuario WHERE idusuario='".$row[8]."'";
...
<?php			
			for ($ii=1; $ii<=$FormatoGrilla[9]['NB']; $ii++)
            {
				...
				if (($Condicion && $row[7]==$_SESSION['id_user'] || 1==$_SESSION['Admin']) || ($FormatoGrilla[9]['BtnId'.$ii]=='BtnVer')){?>
